...
export default class AuroraIMUI extends Component {
  constructor(props) {
    ...
    this._messageListScrollToBottom = this._messageListScrollToBottom.bind(
      this
    );
    ...
    this.setState(
      ...
      () => {
        if (isNeedKeepCusrrentPosition) {
          ...
          const topRowOffset = this.messageList.messageListRef._virtualRenderer
            .getLayoutManager()
            .getOffsetForIndex(topRowIndex);
          this.messageList.messageListRef._virtualRenderer
            .getLayoutManager()
            .getOffsetForIndex(topRowIndex);
          const diff = currentOffset - topRowOffset.y;

          this.messageList.messageListRef._checkAndChangeLayouts(
            this.messageList.messageListRef.props
          );
          const topRowNewOffset = this.messageList.messageListRef._virtualRenderer
            .getLayoutManager()
            .getOffsetForIndex(newMsgs.length);
          this.messageList.messageListRef.scrollToOffset(
            0,
            topRowNewOffset.y + diff
          );
        }
      }
    );
  ...
  updateMessage(msg) {
    if (this.state.messageList) {
      const index = this.state.messageList.findIndex(
        item => msg.msgId === item.values.msgId
      );
  ...
  _messageListScrollToBottom(animated = false) {
    if (
      ...
      const contentHeight = this.messageList.messageListRef._virtualRenderer.getLayoutDimension()
        .height;
      this.messageList.messageListRef.scrollToOffset(
        0,
        contentHeight - this.messageListHeight,
        animated
      );
    }
  }
  ...
  render() {
    const { inputViewStyle = {} } = this.props;
    ...
    return (
      <AuroraIMUIContainer style={styles.container} behavior="padding">
        <MessageList
          ref={component => {
            this.messageList = component;
          }}
          messageList={this.state.messageList}
          extendedState={this.state.extendedState}
